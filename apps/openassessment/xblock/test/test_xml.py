...
from openassessment.xblock.xml import (
    ...
    UpdateFromXmlError, InvalidRubricError, InvalidAssessmentsError, InvalidDatesError
)
...
class TestUpdateFromXml(TestCase):
    ...
    def test_invalid_assessment(self, data):
        # Plug in an assessment validator that always reports
        # that the assessment dictionaries are invalid.
        with self.assertRaises(InvalidAssessmentsError):
            update_from_xml_str(
                self.oa_block, "".join(data['xml']),
                assessments_validator=lambda _: (False, '')
            )

    @file_data('data/update_from_xml.json')
    def test_invalid_dates(self, data):
        # Plug in a date validator that always reports
        # that the assessment dictionaries are invalid.
        with self.assertRaises(InvalidDatesError):
            update_from_xml_str(
                self.oa_block, "".join(data['xml']),
                dates_validator=lambda start, end, dates: (False, '')
            )
