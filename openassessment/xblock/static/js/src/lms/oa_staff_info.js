(function(OpenAssessment) {
    ...
    OpenAssessment.StaffInfoView.prototype = {
        ...
        /**
         Load the staff debug section.
         **/
        load: function() {
            var view = this;

            // If we're course staff, the base template should contain a section
            // for us to render the staff debug section to.  If that doesn't exist,
            // then we're not staff, so we don't need to send the AJAX request.
            if ($('#openassessment__staff-debug', view.element).length > 0) {
                this.server.render('staff_debug').done(
                    function(html) {
                        // Load the HTML and install event handlers
                        $('#openassessment__staff-debug', view.element).replaceWith(html);
                        view.server.renderLatex($('#openassessment__staff-debug', view.element));
                        view.installHandlers();
                    }
                ).fail(
                    function() {
                        view.baseView.showLoadError('staff_debug');
                    }
                );
            }
        },
