...
/*
OpenAssessment.AIExampleListener = function() {
    this.element = $('#openassessment_ai_editor_menu_and_editor');
    this.alert = new OpenAssessment.ValidationAlert();
};

OpenAssessment.AIExampleListener.prototype = {
    /**
     Event handler for updating training examples when a criterion option has
     been updated.
    ...
    optionAdd: function(data) {
        ...
        if (criterionAdded) {
            this.displayAlertMsg(
                gettext("Criterion Added"),
                gettext("You've added a criterion. You'll need to select an option for the criterion in the impacted examples. To do this, click the AI tab.")
            );
        }
    },
    ...
    optionRemove: function(data) {
        ...
        if (invalidated) {
            this.displayAlertMsg(
                gettext("Option Deleted"),
                gettext("You've deleted an option. The system has removed that option from its criterion in the sample responses in the example based assessment step. You may have to select a new option for the criterion.")
            );
        }
    },

    _optionSel: function(criterionName) {
        return $('.openassessment_ai_example_criterion_option[data-criterion="' + criterionName + '"]', this.element);
    },
    ...
    removeAllOptions: function(data) {
        var changed = false;
        $('.openassessment_ai_example_scored_rubric_criterion', this.element).each(function() {
            var criterion = this;
            if ($(criterion).data('criterion') == data.criterionName) {
                $(criterion).remove();
                changed = true;
        ...
        if (changed) {
            this.displayAlertMsg(
                gettext("Option Deleted"),
                gettext("You've deleted all the options for this criterion. The system has removed the criterion from the sample responses in the Example based grading tab.")
            );
        }
    },
    ...
    criterionRemove: function(data) {
        var changed = false;
        var sel = '.openassessment_ai_example_scored_rubric_criterion[data-criterion="' + data.criterionName + '"]';
        $(sel, this.element).each(
            function() {
                $(this).remove();
                changed = true;
        ...
        if (changed) {
            this.displayAlertMsg(
                gettext("Criterion Deleted"),
                gettext("You've deleted a criterion. The system has removed the criterion from the sample responses in the Example Based Grading step.")
            );
        }
    },
    ...
    displayAlertMsg: function(title, msg) {
        if ($("#include_ai_assessment", this.element).is(":checked") &&
            // Check for at least more than one example, to exclude the template
            $(".openassessment_ai_example", this.element).length > 1) {
            this.alert.setMessage(title, msg).show();
        }
    },
