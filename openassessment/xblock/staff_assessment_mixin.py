...
class StaffAssessmentMixin(object):
    ...
    def staff_assess(self, data, suffix=''):
        ...
        try:
            ...
            self.publish_assessment_event("openassessmentblock.staff_assessmet", assessment)

        except (staff_api.StaffAssessmentRequestError):
            logger.warning(
        ...
        except (staff_api.StaffAssessmentInternalError):
            logger.exception(
                u"An error occurred while submitting a staff assessment "
                u"for the submission {}".format(self.submission_uuid),
            )
