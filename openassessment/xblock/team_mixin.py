...
class TeamMixin:
    ...
    def teams_service(self):
        try:
            ...
            logger.error(f'{self.location}: Teams service unavailable')
            raise
    ...
    def teams_configuration_service(self):
        try:
            ...
            logger.error(f'{self.location}: Teams Configuration service unavailable')
            raise
    ...
    def get_team_for_anonymous_user(self, anonymous_user_id):
        ...
        if not user:
            logger.error(f'{self.location}: User lookup for anonymous_user_id {anonymous_user_id} failed')
